...
class miWrapper:
    ...
    def fit(self, bags, labels, n_epoch=100, batch_size=128, dropout=0, weight_decay=0, lr=0.001, verbose=False):
        ...
        self.estimator.fit(bags_modified, labels_modified, n_epoch=n_epoch, batch_size=batch_size,
                           dropout=dropout, weight_decay=weight_decay, lr=lr, verbose=verbose)
        return self.estimator
    ...
    def get_instance_predictions(self, bags):
        preds = [self.estimator.predict(bag.reshape(-1, bag.shape[-1])) for bag in bags]
        return np.asarray(preds)
