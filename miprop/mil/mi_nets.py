...
class Pooling(nn.Module):
    ...
    def forward(self, x: torch.Tensor, m: torch.Tensor) ->  torch.Tensor:
        ...
        Ndescr - number of descriptor, i.e. dimensionalty of current input embeddidng

        Examples
        --------
        >>> import torch
        >>> from torch import Tensor, ones
        >>> from miqsar.mil.mi_nets import Pooling
        >>> # create tensor with 4 molecules, 3 conformers per molecule and 3 descriptor. Also we init m with ones,
...
class BagNet(BaseNet):
    ...
    def forward(self, x: torch.Tensor, m: torch.Tensor) -> Tuple[None, torch.Tensor]:
        ...
        >>> from miqsar.mil.mi_nets import BagNet
        >>> x_train = randn((3, 3, 3))
...
class InstanceNet(BaseNet):
    ...
    def forward(self, x: torch.Tensor, m: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        >>> import numpy as np
        >>> from torch import randn
        >>> from miqsar.mil.mi_nets import InstanceNet
        >>> x_train = randn((3, 3, 3))
...
class BagNetClassifier(BagNet, BaseClassifier):
    ...
    >>> from miqsar.mil.mi_nets import BagNetClassifier
...
class InstanceNetClassifier(InstanceNet, BaseClassifier):
    ...
    ----------
    >>> from torch import randn, Tensor
    >>> from miqsar.mil.mi_nets import InstanceNetClassifier
    >>> x_train, y_train = randn((3, 3, 3)), Tensor([1,0,1])
...
class InstanceNetRegressor(InstanceNet, BaseRegressor):
    ...
    ----------
    >>> from torch import randn, Tensor
    >>> from miqsar.mil.mi_nets import InstanceNetRegressor
    >>> x_train, y_train = randn((3, 3, 3)), randn(3)
